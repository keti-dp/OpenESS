...
def calc_health_indicator_mvf(**context):
    ...
    for bank_id in bank_list:
        ...
        for rack_id in rack_list:          
            ...
            mvf_df = filtered_df.query(f"RACK_ID == {rack_id} and RACK_SOC >= {MIN_SOC}  and RACK_SOC <= {MAX_SOC} and BATTERY_STATUS_FOR_RUN == 1 and BATTERY_STATUS_FOR_CHARGE == 3").reset_index(drop=True)
            mvf_df["RACK_VOLTAGE"] = abs(mvf_df["RACK_VOLTAGE"]-nominal_rack_voltage)
            
            # TODO: baekma의 경우 mvf 데이터가 전혀 안잡히므로 확인이 필요
            print(mvf_df)
...
with DAG(dag_id='baekma_HI_calc_MVF',
