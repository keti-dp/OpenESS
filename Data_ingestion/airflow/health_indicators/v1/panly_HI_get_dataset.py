...
def execute_select_query(**context):
    ...
    try:
        ...
        df.to_feather(DATA_SAVE_PATH + filename + ".feather")
...
with DAG(dag_id='panly_HI_get_dataset',
         default_args=default_args,
         start_date=datetime(2024, 4, 17, tzinfo=kst),
         schedule_interval='00 05 * * *',
         tags=['panly', 'indicator', 'rack', 'dataset'],
         catchup=True
         ) as dag:
    ...
    trigger_TIExVD = TriggerDagRunOperator(
        ...
        trigger_dag_id="panly_HI_calc_TIExVD",
    ...
    trigger_PE = TriggerDagRunOperator(
        task_id="trigger_PE",
        trigger_dag_id="panly_HI_calc_PE",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_MVF = TriggerDagRunOperator(
        task_id="trigger_MVF",
        trigger_dag_id="panly_HI_calc_MVF",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_DCIR = TriggerDagRunOperator(
        ...
        trigger_dag_id="panly_HI_calc_DCIR",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_VIExTD = TriggerDagRunOperator(
        ...
        trigger_dag_id="panly_HI_calc_VIExTD",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
...
start >> clac_past_time >> query_hi_dataset >> branch_operator

branch_operator >> [trigger_TIExVD, trigger_PE, trigger_MVF, trigger_DCIR, trigger_VIExTD] >> complete_HI
branch_operator >> end