    ...
    for bank_id in bank_list:
        ...
        for rack_id in rack_list:
            try:
                ...
                viectd_df = filtered_df.query(f"RACK_ID == {rack_id} and BATTERY_STATUS_FOR_RUN == 1 and BATTERY_STATUS_FOR_CHARGE == 2").reset_index(drop=True)
                viedtd_df = filtered_df.query(f"RACK_ID == {rack_id} and BATTERY_STATUS_FOR_RUN == 1 and BATTERY_STATUS_FOR_CHARGE == 3").reset_index(drop=True)
...
with DAG(dag_id='baekma_HI_calc_VIExTD',
