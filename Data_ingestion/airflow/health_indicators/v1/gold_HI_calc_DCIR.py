    ...
    for bank_id in bank_list:
        ...
        for rack_id in rack_list:          
            try:
                ...
                standby_df = filtered_df.query(f"RACK_ID == {rack_id} and BATTERY_STATUS_FOR_STANDBY == 1").reset_index(drop=True)
                cdcir_df = filtered_df.query(f"RACK_ID == {rack_id} and BATTERY_STATUS_FOR_CHARGE == 1").reset_index(drop=True)
                ddcir_df = filtered_df.query(f"RACK_ID == {rack_id} and BATTERY_STATUS_FOR_DISCHARGE == 1").reset_index(drop=True)
...
with DAG(dag_id='gold_HI_calc_DCIR',
         default_args=default_args,
         start_date=datetime(2023, 2, 1, tzinfo=kst),
