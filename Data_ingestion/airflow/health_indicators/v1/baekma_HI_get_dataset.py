...
with DAG(dag_id='baekma_HI_get_dataset',
         default_args=default_args,
         start_date=datetime(2024, 5, 27, tzinfo=kst),
         schedule_interval='10 05 * * *',
         tags=['baekma', 'indicator', 'rack', 'dataset'],
         catchup=True
         ) as dag:
    ...
    trigger_TIExVD = TriggerDagRunOperator(
        ...
        trigger_dag_id="baekma_HI_calc_TIExVD",
    ...
    trigger_PE = TriggerDagRunOperator(
        task_id="trigger_PE",
        trigger_dag_id="baekma_HI_calc_PE",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_MVF = TriggerDagRunOperator(
        task_id="trigger_MVF",
        trigger_dag_id="baekma_HI_calc_MVF",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_DCIR = TriggerDagRunOperator(
        ...
        trigger_dag_id="baekma_HI_calc_DCIR",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
    ...
    trigger_VIExTD = TriggerDagRunOperator(
        ...
        trigger_dag_id="baekma_HI_calc_VIExTD",
        execution_date="{{ execution_date }}",
        conf='{"filename": "{{ task_instance.xcom_pull(task_ids="query_hi_dataset", key="filename") }}"}'
    )
