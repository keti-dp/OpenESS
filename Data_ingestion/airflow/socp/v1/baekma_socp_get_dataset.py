...
OUTPUT_PATH = "/home/jpark/airflow/dags/calc_socp/dataset/baekma/"
...
def execute_select_query(**context):
    ...
    pg_hook = PostgresHook(postgres_conn_id='baekma_site')
...
with DAG(dag_id='baekma_SoCP_GetDataset',
         default_args=default_args,
         start_date=datetime(2024, 1, 1, tzinfo=kst),
         schedule_interval='10 4 * * *',
         tags=['baekma', 'dataset', 'socp'],
         catchup=True
         ) as dag:
    ...
    trigger_socp_count = TriggerDagRunOperator(
        ...
        trigger_dag_id = "baekma_SoCP_count",
    ...
    trigger_socp_info = TriggerDagRunOperator(
        ...
        trigger_dag_id = "baekma_SoCP_info",
        execution_date = "{{ execution_date }}",
        conf = '{"filename": "{{ task_instance.xcom_pull(task_ids="get_data", key="filename") }}"}'
    )
...
start >> calc_past_time >> get_data >> branch_operator
branch_operator >> [trigger_socp_count, trigger_socp_info] >> complete_socp
branch_operator >> end
