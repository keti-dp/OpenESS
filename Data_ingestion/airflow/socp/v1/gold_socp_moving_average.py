...
SAVE_DATA_PATH = "/home/jpark/airflow/dags/calc_socp/dataset/gold/count/ma/"
...
def get_dataset(start, end, period, **context):
    ...
    query = f"""SELECT * FROM ess_socp_count_oper3 where ("TIMESTAMP" between '{start}' and '{end}')"""
...
with DAG(dag_id='gold_SoCP_moving_average',
    ...
    get_past_5days_dataset = PythonOperator(
        ...
        op_kwargs={
            "start": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['past_5days'] }}",
    ...
    get_past_10days_dataset = PythonOperator(
        ...
        provide_context=True,
        op_kwargs={
            "start": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['past_10days'] }}",
            "end": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['end'] }}",
            "period": 10 
        }
    )
    ...
    get_past_15days_dataset = PythonOperator(
        ...
        provide_context=True,
        op_kwargs={
            "start": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['past_15days'] }}",
            "end": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['end'] }}",
            "period": 15
        }
    )
    ...
    get_past_30days_dataset = PythonOperator(
        ...
        provide_context=True,
        op_kwargs={
            "start": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['past_30days'] }}",
            "end": "{{ task_instance.xcom_pull(task_ids='calc_past_time', key='query_time')['end'] }}",
            "period": 30
        }
    )
    ...
    push_data = PythonOperator(
        ...
        op_kwargs={
            "table_name": "ess_socp_count_ma_oper3",
            "file": "{{ task_instance.xcom_pull(task_ids='get_past_5days_dataset', key='f_name')}}"
        }
    )
...
calc_past_time >> get_past_5days_dataset >> calc_ma_5 >> push_data >> end
calc_past_time >> get_past_10days_dataset >> calc_ma_10 >> push_data >> end
calc_past_time >> get_past_15days_dataset >> calc_ma_15 >> push_data >> end
calc_past_time >> get_past_30days_dataset >> calc_ma_30 >> push_data >> end