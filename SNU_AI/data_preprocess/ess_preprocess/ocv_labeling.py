...
#import script.preprocessing as prep ##folder_dir
import preprocessing as prep

pd.set_option('mode.chained_assignment',  None)


def OCV_label(data_path, site, save_data_dir=None, save_fig_dir=None, monitor=False):
    '''
    data_path : data path for OCV labeling
    site : sionyu or panli
    save_data_dir : save ocv labeled data
        if None, do not save labeled data
    ...
    '''
    site = site
    file_name = data_path[-19:-15] + data_path[-13:-8]
    monitor = monitor
    ...
    if site == 'sionyu':
        voltage_thres = 808 # threshold for sunny days(=fully-charged day)
...
def R_i_label(df):
    """
    df should have column about 'OCV'
    """
    df['Internal_Resistance'] = (df['BANK_DC_VOLT']-df['OCV_est']) / df['BANK_DC_CURRENT']
    return df
...
def ocv_labeling(data, site, voltage_thres, bank_capacity, b2c_current, b2c_voltage, ocv2soc, soc2ocv, save_fig_dir, file_name, monitor, scaling=True):
    ...
    for date in clear_date:
        clear_data.append(prep.select_oneday_data(data, date=date))
...
#     # 33J spec
#     soc_table = np.arange(1., -0.01, -0.05)
#     ocv_table = np.array([4.036, 3.985, 3.941, 3.899, 3.842,
#                         3.809, 3.778, 3.731, 3.693, 3.672,
#                         3.656, 3.637, 3.626, 3.615, 3.599,
#                         3.577, 3.548, 3.504, 3.463, 3.427, 3.213])
#     ocv2soc = interpolate.interp1d(ocv_table, soc_table, kind=inter_k)
#     soc2ocv = interpolate.interp1d(soc_table, ocv_table, kind=inter_k)
...
#                           3.829, 3.792, 3.731, 3.678, 3.650,
#                           3.631, 3.615, 3.601, 3.586, 3.567,
#                           3.539, 3.503, 3.450, 3.418, 3.378, 2.851,])
#     ocv2soc = interpolate.interp1d(ocv_table, soc_table, kind=inter_k)
#     soc2ocv = interpolate.interp1d(soc_table, ocv_table, kind=inter_k)
