...
uint16_t u16_normalTaskClassification = 0;
...
uint8_t u8_taskRandom = 0;
uint32_t u32_delayTime = 0;
uint32_t u32_lastChangeTime = 0;
uint8_t u8_tenSecondCounter = 0;
uint8_t u8_check=0;
...
void Monitor_TASK(void const * argument)//monitor
{
    ...
    while(1)
    {
...
#if 0
    	if(u16_normalTaskClassification <= 250)
    	{
    		u8_taskRandom =(HAL_RNG_GetRandomNumber(&hrng) % 3);
    	}
    	else if(u16_normalTaskClassification > 250 && normal_random_parameter > 990)
    	{
    		u8_taskRandom =(HAL_RNG_GetRandomNumber(&hrng) % 7);

    		if(u8_check == 0)
			{
				u32_lastChangeTime = HAL_GetTick();
				u8_check = 1;
			}
    	}
    	if(u8_check == 1 && (HAL_GetTick() - u32_lastChangeTime) > 10000)
		{
    		u8_taskRandom =(HAL_RNG_GetRandomNumber(&hrng) % 3);
			u16_normalTaskClassification = 0;
			u8_check = 0;
		}
#endif
    	usage = osGetCPUUsage();
		...
		else if(u16_normalTaskClassification > 250 && normal_random_parameter > 800)
		{
			if(u8_check == 0)
			{
				u32_lastChangeTime = HAL_GetTick();
...
#endif
		u16_normalTaskClassification++;
		vTaskPrioritySet(TaskHandle1, u8_taskRandom);
		osDelay(u32_delayTime);
	}
}
...
static void Calc_TASK(void const * argument)
{
    while(1)
    {
		...
		u16_normalTaskClassification++;
		vTaskPrioritySet(TaskHandle2, u8_taskRandom);
    	osDelay(u32_delayTime);
    }
}
...
void Datatx_TASK(void const * argument)
{
	while(1)
	{
		...
		u16_normalTaskClassification++;
		vTaskPrioritySet(TaskHandle3, u8_taskRandom);
    	osDelay(u32_delayTime);
	}
}
