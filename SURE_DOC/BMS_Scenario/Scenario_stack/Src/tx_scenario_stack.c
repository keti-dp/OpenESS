...
void increaseStackUsageTemporarily(int depth, int maxDepth)
{
    volatile uint8_t buffer[256]; // 스택 사용량을 증가시키기 위한 로컬 변수
    memset((void*)buffer, 0, sizeof(buffer)); // 사용되는 것처럼 처리

    if(depth < maxDepth)
    {
        increaseStackUsageTemporarily(depth + 1, maxDepth); // 재귀 호출
    }
    // 재귀 호출이 끝나면 스택에서 buffer가 제거됩니다.
}
void Memory_TASK(void const * argument)
{
	while(1)
	{
		...
		if(normal_task_classification <= 250)
		{
			taskID = 0;
			//nothing
		}
		else if(normal_task_classification > 250 && normal_random_parameter > 993)
		{
			increaseStackUsageTemporarily(0, 6); // 조건을 만족할 때 스택 사용량 증가
