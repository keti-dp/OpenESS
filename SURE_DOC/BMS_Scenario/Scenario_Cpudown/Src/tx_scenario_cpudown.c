...
uint16_t normal_random_parameter = 0;
uint32_t u32_delayTime = 100;

void Monitor_TASK(void const * argument) // Monitor Task
{
    while(1)
    {
...
{
    uint32_t loopCounter = 0; // Loop counter to control CPU load
    ...
    while(1)
    {
        ...
        // Example condition to suspend tasks to reduce CPU usage
        if(normal_task_classification > 1000 && normal_random_parameter > 995)
        {
//            taskID = 1;
//            vTaskSuspend(TaskHandle1);
//            vTaskSuspend(TaskHandle2);
//            osDelay(1000); // Adjust the delay to achieve 10% CPU usage
...
static void Calc_TASK(void const * argument)
{
    while(1)
    {
        ...
        // Suspend tasks to reduce CPU usage
//        if(normal_task_classification > 1000 && normal_random_parameter > 990)
//        {
//            taskID = 2;
//            vTaskSuspend(TaskHandle1);
