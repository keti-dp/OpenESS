...
1. **Katib 하이퍼파라미터 튜닝** (매월 1일 자동 실행)
   ```bash
   # 수동 실행
   airflow dags trigger katib_tuning_your_site_id
   ```

2. **모델 검증 및 배포** (Katib 완료 후 자동 트리거)
   ```bash
   # 수동 실행
...
### 1. Katib 튜닝 DAG (`dag_katib_tuning.py`)
...
### 2. 모델 검증/배포 DAG (`dag_model_validation_deployment.py`)
