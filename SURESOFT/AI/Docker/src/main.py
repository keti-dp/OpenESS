...
from LSTMUSAD import LSTM_USAD_train, LSTM_USAD_pred
...
def main(options):
    ...
    # Set Window_Size
    if options.model in ["lstm_usad", "usad"]:
        DIM = 128
    else:
        DIM = 10
    ...
    if options.model == "usad":
        if options.mode == "train":
            ...
            with open("./../logs/usad_history.txt", "w") as f:
        ...
        if options.mode == "train":
            ...
            usad = LSTM_USAD_train(BATCH_SIZE, N_EPOCHS, LR)
            ...
            with open("./../logs/lstm_usad_history.txt", "w") as f:
                f.write(json.dumps(history, indent=4))
            ...
            test_loader, time_index = usad_model.load_dataset(data, FEATURES)
            results = usad_model.anomaly_detection(test_loader)

            time_index = list(np.datetime_as_string(time_index).flatten())
            # print(len(results), len(time_index))
            if len(FEATURES) == 1:
                results = list(map(np.mean, results))
                # results = np.apply_along_axis(np.mean, axis=1, arr=np.array(results))
            result = json.dumps({x: y for x, y in zip(time_index, results)}, indent=4)
            with open("../logs/LSTM USAD_Prediction.txt", "w") as f:
                f.write(result)
        ...
        if options.mode == "train":
            ...
            with open("./../logs/deepant_history.txt", "w") as f:
                f.write(json.dumps(history, indent=4))
            deepant.save_model()
...
if __name__ == "__main__":
    ...
    # Dataset Path
    parser.add_argument(
        "--dataset", default="./data/train.csv", type=str, help="Path to Dataset"
    )
    parser.add_argument(
        "--mode",
        default="train",
        type=str,
        help="Choose train or predict",
        choices=["train", "pred"],
    )
    ...
    parser.add_argument(
        ...
        default="usad",
        type=str,
        help="Choose usad or deepAnt",
        choices=["usad", "deepant", "lstm_usad"],
        required=True,
    )
    ...
    parser.add_argument(
        "--features", default="CPU usage", nargs="+", type=str, help="Columns for Model"
    )
    parser.add_argument(
        "--es_epochs", default=10, type=int, help="Early Stopping Epochs"
    )
