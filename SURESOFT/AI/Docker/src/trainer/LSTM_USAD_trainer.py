...
from net.LSTM_USAD_net import LSTMEncoder, LSTMDecoder


class LSTMUsadModel(nn.Module):
    def __init__(self, w_size, z_size):
        super().__init__()

        self.encoder = LSTMEncoder(w_size, z_size, 1)
        self.decoder1 = LSTMDecoder(z_size, w_size, 1)
        self.decoder2 = LSTMDecoder(z_size, w_size, 1)
    ...
    def training_step(self, batch, n):
        ...
        w1 = self.decoder1(z, seq_len)
        w2 = self.decoder2(z, seq_len)
        w3 = self.decoder2(self.encoder(w1), seq_len)
    ...
    def testing(self, test_loader, alpha=0.5, beta=0.5):
        ...
        with torch.no_grad():
            for batch in test_loader:
                seq_len = batch.shape[1]
                batch = self.to_device(batch, self.device)
                w1 = self.decoder1(self.encoder(batch), seq_len)
                w2 = self.decoder2(self.encoder(w1), seq_len)

                anomaly_score = alpha * torch.mean(
                    (batch - w1) ** 2, axis=1
                ) + beta * torch.mean((batch - w2) ** 2, axis=1)

                results.append((anomaly_score.cpu().detach().numpy()).tolist())

        return results


class EarlyStopping:
    def __init__(self, patience=7, verbose=False, dataset_name="LSTM_USAD", delta=1e-5):
