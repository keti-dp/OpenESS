...
class UsadModel(nn.Module):
    ...
    def evaluate(self, val_loader, n):
        outputs = [self.validation_step(self.to_device(batch,self.device), n) for [batch] in val_loader]
    ...
    def fit(self, epochs, train_loader, val_loader,lr:float = 1e-4, es_epochs = 7):
        ...
        optimizer1 = optim.Adam(list(self.encoder.parameters())+list(self.decoder1.parameters()), lr = lr)
        optimizer2 = optim.Adam(list(self.encoder.parameters())+list(self.decoder2.parameters()), lr = lr)
        scheduler1 = MultiStepLR(optimizer1,milestones=[(epochs)//5, (epochs)//3, (epochs)//2 ], gamma = 0.5) 
        scheduler2 = MultiStepLR(optimizer2,milestones=[(epochs)//5, (epochs)//3, (epochs)//2 ], gamma = 0.5)
    ...
    def testing(self, test_loader, alpha=.5, beta=.5):
        ...
        with torch.no_grad():
            for [batch] in test_loader:
                ...
                anomaly_score = alpha*torch.mean((batch-w1)**2,axis=1)+beta*torch.mean((batch-w2)**2,axis=1)
